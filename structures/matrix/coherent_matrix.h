...
namespace structures
{
    ...
    {
        informationVector_ = new Array<T>(rowCount * columnCount);
        rowCount_ = rowCount;
    ...
    inline CoherentMatrix<T>::CoherentMatrix(CoherentMatrix& other)
    {
        ...
        informationVector_ = new Array<T>(rowCount_ * columnCount_);
        informationVector_->copy(*other.informationVector_, 0, *informationVector_, 0, other.size());
    }

    template<typename T>
    inline CoherentMatrix<T>::~CoherentMatrix()
    {
        delete informationVector_;
        informationVector_ = nullptr;
        columnCount_ = 0;
        rowCount_ = 0;
    }
    ...
    inline Structure& CoherentMatrix<T>::assign(Structure& other)
    {
        if (this != &other)
        {
            auto* otherMatrix = dynamic_cast<CoherentMatrix<T>*>(&other);
...
//                delete informationVector_;
//                informationVector_ = new Array<T>(rowCount_ * columnCount_);
//                informationVector_->copy(*otherMatrix->informationVector_, 0, *informationVector_, 0,
//                                         otherMatrix->size());
                    informationVector_->assign(*otherMatrix->informationVector_);
            } else
            {
    ...
    inline bool CoherentMatrix<T>::equals(Structure& other)
    {
        ...
        if (otherMatrix != nullptr)
        {
            if (otherMatrix->informationVector_->equals(*informationVector_))
            {
                return true;
            } else
            {
    ...
    {
        Utils::rangeCheckExcept(rowIndex, rowCount_, "Invalid row index in coherent_matrix.at()!");
        Utils::rangeCheckExcept(columnIndex, columnCount_, "Invalid column index in coherent_matrix.at()!");

        return informationVector_->at((columnCount_ * rowIndex) + columnIndex);
    }
}