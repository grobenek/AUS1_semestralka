...
namespace structures
{
	...
	{
		KWayTreeNode<T, 2>::replaceSon(leftSon, LEFT_SON); // skontrolovat
	}
	...
	inline void BinaryTree<T>::InOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
        ...
        if (current->getSon(0) != nullptr)
        {
            this->populatePath(current->getSon(0));
        }
        this->path_->push(current);
        if (current->getSon(1) != nullptr)
        {
            this->populatePath(current->getSon(1));
        }
	}
