...
namespace structures
{
	...
	{
		throw std::logic_error("Not supported!"); // aby neboli leaks
	}

	template<typename T, int K>
	inline TreeNode<T>* KWayTreeNode<T, K>::replaceSon(TreeNode<T>* son, int order)
	{
		TreeNode<T>* oldSon = children_->at(order);
		auto* newSon = dynamic_cast<KWayTreeNode<T, K>*>(son);

		children_->at(order) = newSon;
		if (newSon != nullptr)
		{
