...
namespace structures
{
	...
	{
		auto* itemToFind = this->findTableItem(key);

		if (itemToFind != nullptr)
		{
			return itemToFind->accessData();
		}
		else
		{
	...
	{
		auto* item = this->findTableItem(key);
	...
	inline bool SequenceTable<K, T>::tryFind(const K& key, T& data)
	{
		...
		if (item != nullptr)
		{
			data = item->accessData(); // priradenie dat
			return true;
		}
		else
		{
	...
	inline void SequenceTable<K, T>::clear()
	{
		...
		this->list_->clear(); // vycistenie tabulky
	}
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::assignSequenceTable(SequenceTable<K, T>& other)
	{
		if (this != &other)
		{
			...
			for (auto* otherItem : *(other.list_))
			{
